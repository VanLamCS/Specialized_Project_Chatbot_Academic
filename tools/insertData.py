...
def generate_elements(questions_in_origin, add_data_generated, template_element, list_question):
    if template_element is None:
        return []
    questions_generated = []
    for question in list_question:
        if not is_question_exists([add_data_generated, questions_generated, questions_in_origin], question):
...
def main():
    ...
    for add_file_name in add_files:
        if add_file_name.endswith(".json") and add_file_name in dest_files:
            ...
            if not isinstance(add_data, list) and not add_data.strip() or len(add_data) == 0:
                continue
            for set in add_data:
                template_element = find_element_by_id(dest_data["data"], set["id"])
                questions_generated = generate_elements(dest_data["data"], add_data_generated, template_element, set["questions"])
